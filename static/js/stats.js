  ...
  document.getElementById('content').innerHTML = `
    <div class="section">
      ...
      <div class="totals-grid">
        ...
        <div class="stat-card">
          <div class="val">${fmt(t.nb_activities)}</div>
          <div class="unit">activités</div>
        </div>

        <div class="stat-card">
          <div class="val">${fmt(km)}</div>
          <div class="unit">km parcourus</div>
        </div>

        <div class="stat-card">
          <div class="val">${h}h${String(min).padStart(2,'0')}</div>
          <div class="unit">en mouvement</div>
        </div>

        <div class="stat-card">
          <div class="val">${fmt(Math.round(t.total_elevation))}</div>
          <div class="unit">m dénivelé</div>
        </div>

        <div class="stat-card">
          <div class="val">${fmt(Math.round(t.total_calories))}</div>
          <div class="unit">kcal brûlées</div>
        </div>

        ${t.avg_heartrate ? `
          <div class="stat-card">
            <div class="val">${Math.round(t.avg_heartrate)}</div>
            <div class="unit">bpm moyen</div>
          </div>
        ` : ''}
